...
#define BRIGHT_N 40         // яркость ночная, %
...
#define SHOW_TEMP_HUM 0     // 0 - не показывать температуру и вл., 1 - показывать
...
#define SEND_INTERVAL 1     // временной интервал отправки сообщений через последовательный UART интерфейс
...
byte digitsDraw[7];
boolean dotFlag;
int8_t hrs = 10, mins = 10, secs, lastSecondOnSend = 0;
...
void sendTimeToDevice() {
  byte buff[3] = {0}; 
  if ((secs % SEND_INTERVAL) == 0 && secs != lastSecondOnSend) {
    lastSecondOnSend = secs;
    buff[0] = (byte) hrs;
    buff[1] = (byte) mins;
    buff[2] = (byte) secs;
    Serial.write(buff, 3);
